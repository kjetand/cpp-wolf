...
#define TIMEY  8
    ...
    //
    if (static_cast<byte>(gamestate.difficulty) >= static_cast<byte>(difficulty_t::gd_medium)) {
...
#define RATIOXX 37
        ...
        bonus = (int32_t)timeleft * PAR_AMOUNT + (PERCENT100AMT * (kr >= 100)) + (PERCENT100AMT * (sr >= 100)) + (PERCENT100AMT * (tr >= 100));
